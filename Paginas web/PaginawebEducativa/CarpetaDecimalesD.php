<?php require_once("php/ClaseDD.php");
...
if($ejercicio==0){
	$ejercicio++;
 	...
 	if(isset($_POST['comenzar'])){
 		...
 		header('Location: CarpetaDecimalesD.php?visible=2&saludo='.$ejercicio, true);
 	}else{
 		$ventana='none';
 		$boton='block';
 	}
 ...
 if(isset($_POST['mandar'])){
	...
	if($_GET['intentos']<=1 ){
		...
		if($objecto->respuesta($_POST['respuesta'],$_GET['saludo'])){
			...
			//@$bien=1;
					$aciertos+=1;
					header('Location: CarpetaDecimalesD.php?saludo='.$ejercicio.'&estado=1'."&intentos=".$intentos."&visible=2"."&aciertos=".$aciertos, true);

		}else{
			//@$malo=1;
				 header('Location: CarpetaDecimalesD.php?saludo='.$ejercicio.'&estado=2'."&intentos=".$intentos."&visible=2"."&aciertos=".$aciertos, true);			
		}
    }else{
    	if($_GET['intentos']==2 ){
    		if($objecto->respuesta($_POST['respuesta'],$_GET['saludo'])){
			...
			header('Location: CarpetaDecimalesD.php?saludo='.$_GET['saludo'].'&estado=1'."&intentos=".$intentos."&visible=2"."&aciertos=".$aciertos."&bandera=true", true);

		}else{
			//@$malo=1;
				
				 header('Location: CarpetaDecimalesD.php?saludo='.$_GET['saludo'].'&estado=2'."&intentos=".$intentos."&visible=2"."&aciertos=".$aciertos."&bandera=true", true);	
		}
    	}
    }
 }
...
setTimeout(function(){
  ...
  typeEffect(p, speed);
		...
		<div id= "general" >
			<a style="text-decoration: none;" href= "VideoDD.htm">Introducción</a>


			<a style="text-decoration: none;"  class="ejercicios"href= "CarpetaDecimalesD.php">Ejercicios</a>
	...
	<tr >
        ...
        top:470;"><?php echo $intentos." / 3 ";?></h3>
			...
			<a style="text-decoration: none;" class="juegos" href= "JuegoDD.htm">Juegos</a>
      ...
      <div class="modal-content">
       ...
       Incorrectos : <?php echo -($_GET['aciertos']-3);?><br>
        ...
        <div class="modal-footer">
        	<form method="post" action="RDD.php">
       			   <button type="submit" style="padding: 3px 10px; border: PowderBlue 5px double;border-top-left-radius: 5px;
  border-bottom-right-radius: 5px; outline: none; background: #057ccf;" class="btn btn-danger opcion" href= "CarpetaDecimalesD.php" >Aceptar</button>
          </form>
        </div>
